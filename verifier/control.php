<?php
	...
	if(isset($_POST['login']))
	{
		...
		$udata=$obj->selectwhere($connect,"tbl_verifiers",$where);
			
		$count=count($udata);	
		//echo $count;
			if($count>0)
			{
				$_SESSION['verid']=$udata[0]->verid;
				$_SESSION['emailid']=$e;
				$_SESSION['pass']=$p;	
	...
	if(isset($_POST['add_report']))
	{
		...
		$rn=$_POST['rname'];
		$rd=$_POST['rdescription'];
		
		    $data=array("rname"=>$rn,"rdescription"=>$rd);
	
		$obj->insertdata($connect,"tbl_reports",$data);
		header("location:listreports.php");			
	}
	
	$report_all_data=$obj->selectall($connect,"tbl_reports");
	...
	if(isset($_REQUEST['del_report']))
			{
				$did=$_REQUEST['del_report'];
				$where=array("rid"=>$did);
				
				$obj->deletedata($connect,"tbl_reports",$where);	
				header('location:listreports.php');
			}
	$fudata=$obj->selectall($connect,"tbl_reports");
	...
	/*edit a report*/
	if(isset($_REQUEST['edit_report']))
	{
		$report_id=$_GET['edit_report'];
		$where=array("rid"=>$report_id);
		$fetchr=$obj->selectwhere($connect,"tbl_reports",$where);
		...
		$obj->updateddata($connect,"tbl_reports",$data,$where);
		header("location:listreports.php");			
		}
